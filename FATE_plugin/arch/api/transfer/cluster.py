...
# For profiling
import profile.net_tools as net_tools
...
async def _async_receive(stub, transfer_meta):
    ...
    # net profile
    stater = net_tools.netstat(transfer_meta.job.jobId, transfer_meta.job.name, 'receive', transfer_meta.tag, transfer_meta.src.partyId, transfer_meta.src.name, transfer_meta.dst.partyId, transfer_meta.dst.name)
    stater.tick()
    # net profile
...
class FederationRuntime(object):
    ...
    def remote(self, obj, name: str, tag: str, role=None, idx=-1):
        ...
        for _role, _partyIds in parties.items():
            for _partyId in _partyIds:
                ...
                # net profile
                stater = net_tools.netstat(self.job_id, name, 'send', tag, self.party_id, self.role, _partyId, _role)
                stater.add_size(obj)
                stater.tick()
                # net profile
    ...
    def get(self, name, tag, idx=-1):
        ...
        for recv_meta in results:
            ...
            # net profile
            stater = net_tools.netstat(recv_meta.job.jobId, recv_meta.job.name, 'get', recv_meta.tag, recv_meta.src.partyId, recv_meta.src.name, recv_meta.dst.partyId, recv_meta.dst.name)
            stater.tick()
            # net profile
