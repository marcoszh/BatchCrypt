    ...
    def fit(self, data_instances, validate_data=None):
        ...
        for iter_num in range(self.max_iter):
            ...
            for train_x, train_y in self.zcl_dataset:
                ...
                host_max = [np.max(layer) for layer in grads]
                host_min = [np.min(layer) for layer in grads]

                # clipping_thresholds_host = batch_encryption.calculate_clip_threshold_aciq_l(grads,
                #                                                                             bit_width=self.bit_width)
                # LOGGER.debug('clipping shape ' + str(len(clipping_thresholds_host)))

                self.transfer_variable.host_grad_max.remote(obj=host_max, role=consts.GUEST, idx=0,
                                                            suffix=(iter_num_, batch_num))
                self.transfer_variable.host_grad_min.remote(obj=host_min, role=consts.GUEST, idx=0,
                                                            suffix=(iter_num_, batch_num))

                clipping_thresholds = self.transfer_variable.clipping_threshold.get(idx=0, suffix=(iter_num_, batch_num))
                ...
                # self.transfer_variable.host_grad.remote(obj=grads.for_remote(), role=consts.ARBITER, idx=0,
                #                                         suffix=(iter_num_, batch_num))
                self.transfer_variable.host_grad.remote(obj=enc_grads, role=consts.ARBITER, idx=0,
                                                        suffix=(iter_num_, batch_num))
                LOGGER.info("Sent grads")
                self.transfer_variable.host_loss.remote(obj=loss_value, role=consts.ARBITER, idx=0,
                                                        suffix=(iter_num_, batch_num))
