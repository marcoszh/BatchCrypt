...
class HomoLRArbiter(HomoLRBase):
    ...
    def fit(self, data_instances=None, validate_data=None):
        ...
        self.transfer_variable.num_party.remote(obj=(0, num_host + 1), role=consts.GUEST, idx=0, suffix=('train',))
        ...
        for _idx, _cipher in enumerate(host_ciphers):
            self.transfer_variable.num_party.remote((_idx + 1, num_host + 1), role=consts.HOST, idx=_idx, suffix=('train',))
        ...
        for iter_num in range(self.max_iter):
            ...
            while batch_num >= 0:
                ...
                guest_grad = np.array(guest_grad)
                ...
                host_grads = [np.array(x) for x in host_grads]
                ...
                sum_grads = self.__aggregate_grads(host_grads)
                # sum_grads = batch_encryption.aggregate_gradients_old(host_grads)
                sum_loss = self.__aggregate_losses(host_losses)

                LOGGER.info("Grads and loss aggregated")
                # sum_grads = Gradients(sum_grads)
                self.transfer_variable.aggregated_grad.remote(obj=sum_grads, role=consts.GUEST, idx=0,
                                                              suffix=(iter_num, batch_num))
                self.transfer_variable.aggregated_grad.remote(obj=sum_grads, role=consts.HOST, idx=-1,
                                                              suffix=(iter_num, batch_num))
